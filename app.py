...
ID_CARPETA_DRIVE = "1NQm87zuymb6w9Kn8OAhB8VgxNPmoGgZd"

# ==========================================
# 2. BASE DE DATOS GLOBAL Y LOGIN (GOOGLE SHEETS & DRIVE)
# ==========================================
def conectar_servicios():
    try:
        ...
        hoja = client.open("Trading_Journal_DB").sheet1
        servicio_drive = build('drive', 'v3', credentials=creds)
        return hoja, servicio_drive
    except Exception as e:
        return None, None

hoja_excel, drive_service = conectar_servicios()
...
def subir_a_google_drive(archivo_streamlit):
    ...
    try:
        file_metadata = {
            'name': f"journal_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{archivo_streamlit.name}",
            'parents': [ID_CARPETA_DRIVE]
        }
        media = MediaIoBaseUpload(io.BytesIO(archivo_streamlit.getvalue()),
                                  mimetype=archivo_streamlit.type,
                                  resumable=True)
