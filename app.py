...
# --- 2. RESETEAR CUENTA ACTUAL (FUERA) ---
...
with st.sidebar.expander(f"🔄 Reset {ctx_actual}"):
    ...
    if st.button("🔄 Confirm Reset", use_container_width=True, key="btn_solicitar_reset"):
        st.session_state.confirm_reset = True
        
    if st.session_state.confirm_reset:
        st.warning(f"Reset {ctx_actual}?")
        cr_yes, cr_no = st.columns(2)
        if cr_yes.button("YES, RESET", key="btn_si_reset_final"):
...
with st.sidebar.expander("🗑️ Delete Account"):
    ...
    if st.button("🗑️ Delete Selected", use_container_width=True, key="btn_solicitar_borrado"):
        if len(db_usuario) <= 1:
            st.error("Cannot delete your only account.")
        else:
            st.session_state.confirm_delete_acc = True
            
    if st.session_state.confirm_delete_acc:
        st.warning(f"Delete '{cuenta_a_borrar}'?")
        cd_yes, cd_no = st.columns(2)
        if cd_yes.button("YES, DELETE", key="btn_si_borrar_final"):
            ...
            st.session_state.confirm_delete_acc = False
            st.rerun()
        if cd_no.button("CANCEL", key="btn_no_borrar_final"):
            st.session_state.confirm_delete_acc = False
            st.rerun()
...
with st.form(key="form_main_entry", clear_on_submit=True, border=False):
    ...
    if btn_save:
        ...
        if entrada_limpia == "":
            st.error("⚠️ The box cannot be empty.")
        else:
...
with col_cal:
    if paso_cuenta:
        ...
        with c_p1:
            st.markdown(f'<div style="font-size: 20px; font-weight: 700; color: #000000; margin-bottom: 5px;">Amount To Withdraw</div>', unsafe_allow_html=True)
        ...
        with c_p2: st.markdown(f'<div style="{e_caja_p}"><div style="font-size: 20px; font-weight: 700; color: gray;">Total Withdrawn</div><div style="color: #00C897; font-size: 26px; font-weight: 800;">${total_retirado:,.2f}</div></div>', unsafe_allow_html=True)
        with c_p3: st.markdown(f'<div style="{e_caja_p}"><div style="font-size: 20px; font-weight: 700; color: gray; ">Total Withdrawals</div><div style="color: {c_dash}; font-size: 26px; font-weight: 800;">{retiros_realizados}</div></div>', unsafe_allow_html=True)
        with c_p4: st.markdown(f'<div style="{e_caja_p}"><div style="font-size: 20px; font-weight: 700; color: gray; ">Days Done</div><div style="color: #00C897; font-size: 26px; font-weight: 800;">{dias_ganadores_count}</div></div>', unsafe_allow_html=True)
