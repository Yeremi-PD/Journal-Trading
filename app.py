...
# Función de imágenes por archivo desactivada.
...
# 🛡️ SEGURIDAD EXTREMA: Bloquear enlaces guardados si el usuario fue baneado
    try:
        ...
        if not acceso_permitido:
            mostrar_pantalla_bloqueo(query_u)
    except NameError:
        pass
...
/* FIX MÓVIL PARA LOGIN EXTREMO: Destrucción de columnas nativas */
    ...
    st.markdown("<div class='login-title'>PF Journal Pro</div>", unsafe_allow_html=True)
    st.markdown("<div class='login-sub'>Tu Journal avanzado de trading</div>", unsafe_allow_html=True)
...
# 🛡️ Verificar Licencia en vivo antes de dejarlo entrar
                            ...
                            if not acceso_permitido:
                                mostrar_pantalla_bloqueo(user_match)
...
# --- SEGURIDAD: Expulsar si el perfil visitado no tiene cuentas públicas ---
if st.session_state.get("viewing_user"):
    cuentas_publicas = db_global[st.session_state.viewing_user]["settings"]["PC"].get("public_accounts", [])
    if not cuentas_publicas:
        st.session_state.viewing_user = None 
    elif st.session_state.get("data_source_sel") not in cuentas_publicas:
        st.session_state.data_source_sel = cuentas_publicas[0]
...
if not db_usuario or (len(db_usuario) == 1 and "Todas las Cuentas" in db_usuario):
    ...
    st.markdown("<div class='setup-main-title'>PF Journal Pro</div>", unsafe_allow_html=True)
...
def modal_galeria_individual(ctx):
    ...
    opciones = ["Todas", "Eval", "Funded"]
    idx_def = 2 if estado_actual == "Funded" else 1
...
# 🚫 Bloqueamos la conversión a cuenta fondeada (Funded) si estamos en Backtesting o en Todas las Cuentas
...
# El estatus Funded/Eval se calcula siempre para que veas si la cuenta es fondeada o no
if paso_cuenta:
...
if True:
    ...
    with col_data: 
        ...
        if modo_lectura:
            opciones_cta = db_global[usuario]["settings"]["PC"].get("public_accounts", [])
            if not opciones_cta: opciones_cta = ["Privado"]
        else:
            opciones_cta = ["Todas las Cuentas"] + [c for c in db_usuario.keys() if c != "Todas las Cuentas"]
        
        idx_sel = opciones_cta.index(st.session_state.data_source_sel) if st.session_state.data_source_sel in opciones_cta else 0
        st.selectbox("Data Source", opciones_cta, index=idx_sel, key="data_source_sel", label_visibility="collapsed")
    ...
    /* Títulos pequeños de arriba (Date:, P&L:, etc.) */
    .lbl-header {
...
if es_admin:
    with tab_asistente:
        ...
        with col_chat:
            # Mostrar el historial de la conversación seleccionadaalisis
...
with tab_tabla:
    with st.container():
        ...
        if not all_trades: 
            st.markdown("<div style='text-align: center; padding: 50px; background: #1E293B; border-radius: 12px; border: 1px dashed #334155;'><h3 style='color: #F8FAFC;'>Tabla Vacía 📊</h3><p style='color: #94A3B8;'>Registra operaciones para generar tu reporte tabular.</p></div>", unsafe_allow_html=True)
        else:
...
with tab_galeria:
    ...
    cnt_pa = len([x for x in todas_imagenes if x[3] == "Funded"])
...
📸 Total Imágenes: <span>{cnt_todas}</span> | <span class="txt-eval">Eval:</span> <span>{cnt_eval}</span> | <span class="txt-pa">Funded:</span> <span>{cnt_pa}</span>
</div>
<div class="gal-grid">
