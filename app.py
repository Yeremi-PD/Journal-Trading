...
# ==========================================
# 3. CONSTANTES Y DISEÑO
# ==========================================
TEMA_POR_DEFECTO = "Oscuro"
TXT_DASHBOARD = "Dashboard"
TXT_DASH_SIZE, TXT_DASH_X, TXT_DASH_Y = 60, 20, -20         
...
OPT_DATA_1, OPT_DATA_2, OPT_DATA_SIZE = "Account Real", "Account Demo", 14    
OPT_DATA_COLOR_C, OPT_DATA_COLOR_O = "#000000", "#FFFFFF"
...
BALANCE_SIZE, BALANCE_BOX_W, BALANCE_BOX_X, BALANCE_BOX_Y = 30, 50, 0, 0     
...
TXT_MES_SIZE, TXT_MES_COLOR_C, TXT_MES_COLOR_O = 28, "#000000", "#FFFFFF"
TXT_DIAS_SEM_SIZE, TXT_DIAS_SEM_COLOR_C, TXT_DIAS_SEM_COLOR_O = 15, "#000000", "#FFFFFF"

TXT_NUM_DIA_SIZE, TXT_NUM_DIA_COLOR_C, TXT_NUM_DIA_COLOR_O = 20, "#000000", "#c0c0c0"
TXT_PNL_DIA_SIZE, TXT_PCT_DIA_SIZE = 30, 25
TXT_PCT_DIA_COLOR_C, TXT_PCT_DIA_COLOR_O = "#000000", "#000000"

BTN_CAM_EMOJI, BTN_CAM_SIZE, BTN_CAM_X, BTN_CAM_Y = "📷", 30, 0, 2
BTN_CAM_BG_C, BTN_CAM_BG_O = "rgba(255,255,255,0.8)", "rgba(0,0,0,0.6)"
TXT_CERRAR_MODAL = "✖ CERRAR"

CARD_PNL_TITULO, CARD_PNL_TITULO_SIZE, CARD_PNL_TITULO_COLOR_C, CARD_PNL_TITULO_COLOR_O = "Net P&L", 20, "#000000", "#FFFFFF"
CARD_PNL_W, CARD_PNL_H, CARD_PNL_X, CARD_PNL_Y = "100%", "auto", 0, 0          
CARD_WIN_TITULO, CARD_WIN_TITULO_SIZE, CARD_WIN_TITULO_COLOR_C, CARD_WIN_TITULO_COLOR_O = "WinRate", 20, "#000000", "#FFFFFF"
...
WEEKS_TITULOS_SIZE, WEEKS_TITULOS_COLOR_C, WEEKS_TITULOS_COLOR_O = 20, "#000000", "#FFFFFF"
WEEKS_VALOR_SIZE, WEEKS_PCT_SIZE = 25, 20          
WEEK_BOX_W, WEEK_BOX_H, MONTH_BOX_W, MONTH_BOX_H = "31%", "120px", "100%", "120px"        
MONTH_TITLE_SIZE, MONTH_VAL_SIZE = 30, 25         
...
if "Real Data" in db_usuario: db_usuario["Account Real"] = db_usuario.pop("Real Data")
if "Demo Account" in db_usuario: db_usuario["Account Demo"] = db_usuario.pop("Demo Account")
...
def procesar_cambio():
    ...
    if nuevo != viejo:
        ...
        db_usuario[ctx]["trades"][clave] = {
            ...
            "imagenes": old_trade_data.get("imagenes", []),
...
st.markdown(f"""
    ...
    div[data-testid="stNumberInput"] input {{ color: {c_lbl_in} !important; font-size: {INPUT_BAL_TXT_SIZE}px !important; font-weight: bold !important; height: {INPUT_BAL_H} !important; }}
    ...
    [data-testid="stFileUploadDropzone"] {{ background-color: {drop_bg} !important; border: {drop_border} !important; border-radius: 10px !important; width: {DROPZONE_W} !important; height: {DROPZONE_H} !important; display: flex !important; justify-content: center !important; align-items: center !important; }}
    ...
    [data-testid="stFileUploadDropzone"] button {{ background-color: {u_btn_bg} !important; border: 1px solid {border_color} !important; border-radius: 6px !important; width: {BTN_UP_W} !important; height: {BTN_UP_H} !important; }}
    [data-testid="stFileUploadDropzone"] button::after {{ content: "{BTN_UP_TEXTO}" !important; font-size: {BTN_UP_SIZE} !important; color: {u_btn_txt} !important; }}
    [data-testid="stFileUploadDropzone"] button div {{ display: none !important; }}

    div[data-testid="stPopover"] > button {{ min-height: {BTN_CAL_H}px !important; width: {BTN_CAL_W}px !important; background-color: {btn_bg} !important; color: {btn_txt} !important; border: 1px solid {border_color} !important; font-size: {BTN_CAL_ICON_SIZE}px !important; }}
    ...
    .card {{ aspect-ratio: 1 / 1 !important; padding: 5px !important; border-radius: 20px !important; display: flex !important; flex-direction: column !important; position: relative !important; margin-bottom: 6px !important; padding-bottom: 25px !important; }}
    ...
    .day-pct {{ font-size: {TXT_PCT_DIA_SIZE}px !important; color: {c_pct_dia} !important; font-weight: 600 !important; display: block !important; }}
    
    .cam-icon {{ position: absolute !important; bottom: {BTN_CAM_Y}px !important; left: 50% !important; transform: translateX(calc(-50% + {BTN_CAM_X}px)) !important; font-size: {BTN_CAM_SIZE}px !important; cursor: pointer !important; background: {c_cam_bg} !important; border-radius: 50% !important; padding: 2px 4px !important; transition: 0.2s !important; }}
    ...
    .fs-modal img {{ max-width: 90vw !important; max-height: 80vh !important; box-shadow: 0 0 20px black !important; border-radius: 10px !important; object-fit: contain !important; }}
    .close-btn {{ color: white !important; font-size: 25px !important; position: absolute !important; top: 30px !important; right: 50px !important; cursor: pointer !important; background: red !important; padding: 5px 15px !important; border-radius: 8px !important; }}
    ...
    .pnl-value {{ font-size: 28px !important; font-weight: 800 !important; color: #00C897 !important; }}
    ...
    .win-value {{ font-size: {CARD_WIN_VALOR_SIZE}px !important; font-weight: 800 !important; color: {c_val_win} !important; }}
    ...
    /* CSS PARA EL BOTÓN 📝 TRANSPARENTE EN LA ESQUINA DEL DÍA */
...
# ==========================================
# 7. HEADER (BARRA SUPERIOR E INPUTS)
# ==========================================
...
with c_img:
    ...
    if archivos:
        ...
        lista_b64 = [f"data:{img.type};base64,{convertir_img_base64(img)}" for img in archivos]
        db_usuario[ctx]["trades"][clave_actual]["imagenes"].extend(lista_b64)


# =========================================================================================================
# 🔴 🔴 BLOQUE AISLADO PARA MODIFICAR LAS OPCIONES DEL TRADE 📝 (FUNCIONES) 🔴 🔴
# =========================================================================================================
# (AQUÍ ESTÁN TODAS LAS OPCIONES QUE PEDISTE, SUS COLORES Y TEXTOS, SEPARADOS PARA QUE LO EDITES FÁCIL)
...
def renderizar_detalles_trade(trade_data_ref, modal_key):
    """Función que dibuja TODO el contenido del menú despegable 📝"""
    st.markdown("<h3 style='text-align:center;'>Detalles del Trade</h3>", unsafe_allow_html=True)
    
    # 1. BIAS (Puedes cambiar textos y colores aquí)
    bias_options = [
        {'text': 'ALCISTA', 'color': '#337ab7'}, 
        {'text': 'BAJISTA', 'color': '#777777'}, 
        {'text': 'NEUTRO', 'color': '#8c6e5c'}  
    ]
    colorful_menu(bias_options, f"bias_{modal_key}", "Bias", 'bias', trade_data_ref, modal_key)
    st.markdown("<br>", unsafe_allow_html=True)
    
    # 2. CONFLUENCIAS (Agrega o edita a tu gusto)
    confluencias_options = [
        {'text': '1. BIAS Claro', 'color': '#8c6e5c'}, 
        {'text': '2. Liq Sweep', 'color': '#777777'}, 
        {'text': '4. IFVG', 'color': '#b06c9b'},      
        {'text': '3. FVG', 'color': '#a52a2a'},       
        {'text': 'EQH / EQL', 'color': '#5cb85c'},    
        {'text': 'BSL / SSL', 'color': '#5cb85c'},    
        {'text': 'PO3', 'color': '#b06c9b'},          
        {'text': 'SMT', 'color': '#8e44ad'},          
        {'text': 'Breaker Block', 'color': '#95a5a6'},
        {'text': 'Descuento', 'color': '#b08e33'},    
        {'text': 'Order Block', 'color': '#5cb85c'},  
        {'text': 'NYMO', 'color': '#b06c9b'},         
        {'text': 'PDH', 'color': '#8c6e5c'},          
        {'text': 'PDL', 'color': '#8c6e5c'},          
        {'text': 'Inducement', 'color': '#b06c9b'},   
        {'text': 'Turtle Soup', 'color': '#b06c9b'},  
        {'text': 'Continuación', 'color': '#8e44ad'}, 
        {'text': 'Reversal', 'color': '#8c6e5c'},     
        {'text': 'Data High', 'color': '#a52a2a'},    
        {'text': 'Data Low', 'color': '#a52a2a'},     
        {'text': 'CISD', 'color': '#b08e33'},        
        {'text': 'Nada', 'color': '#d9534f'}          
    ]
    colorful_multiselect(confluencias_options, "Confluencias", 'confluencias', trade_data_ref, modal_key)
    st.markdown("<br>", unsafe_allow_html=True)

    # 3. RAZÓN DEL TRADE (Caja de texto)
    if 'razon_trade' not in trade_data_ref: trade_data_ref['razon_trade'] = ""
    trade_data_ref['razon_trade'] = st.text_area("Razón del Trade", value=trade_data_ref['razon_trade'], key=f"razon_{modal_key}")
    
    # 4. CORRECCIONES (Caja de texto)
    if 'correcciones' not in trade_data_ref: trade_data_ref['correcciones'] = ""
    trade_data_ref['correcciones'] = st.text_area("Correcciones", value=trade_data_ref['correcciones'], key=f"corr_{modal_key}")
    
    # 5. % RISK
    risk_options = [
        {'text': '0.6%', 'color': '#777777'}, 
        {'text': '0.5%', 'color': '#777777'}, 
        {'text': '0.4%', 'color': '#777777'}  
    ]
    ...
    # 7. TRADE TYPE
    if 'trade_type' not in trade_data_ref: trade_data_ref['trade_type'] = ""
    trade_data_ref['trade_type'] = st.text_input("Trade Type", value=trade_data_ref['trade_type'], key=f"type_{modal_key}")
    
    # 8. EMOCIONES (Caja de texto)
    if 'emociones' not in trade_data_ref: trade_data_ref['emociones'] = ""
    trade_data_ref['emociones'] = st.text_area("Emociones", value=trade_data_ref['emociones'], key=f"emoc_{modal_key}")
                ...
                if dia == 0: st.markdown('<div class="card cell-empty"></div>', unsafe_allow_html=True)
                    ...
                    if trade_info and visible:
                        ...
                        with st.popover("📝", key=popover_key):
                            ...
                            renderizar_detalles_trade(trade_data_ref, modal_key)
                        ...
                        st.markdown(f'<div class="card {c_cls}"><div class="day-number">{dia}</div><div class="day-content"><span class="day-pnl">{c_sim}${trade_info["pnl"]:,.2f}</span><br><span class="day-pct">{c_sim}{pct:.2f}%</span></div>{cam_html}</div>', unsafe_allow_html=True)
                    else:
...
with col_det:
    ...
    st.markdown(f'<div class="metric-card card-pnl"><div class="metric-header"><span class="title-net-pnl">{CARD_PNL_TITULO}</span></div><div class="{color_pnl}">{simbolo_pnl}${net_pnl:,.2f}</div></div><br>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="metric-card card-win"><div><div class="metric-header"><span class="title-trade-win">{CARD_WIN_TITULO}</span></div><div class="win-value">{win_pct:.2f}%</div></div><div class="gauge-container">{svg_html}<div class="gauge-labels"><span class="lbl-g">{wins}</span><span class="lbl-b">{ties}</span><span class="lbl-r">{losses}</span></div></div></div>', unsafe_allow_html=True)
    ...
    def get_col_simb(valor): return ("txt-green", "+") if valor > 0 else (("txt-red", "") if valor < 0 else ("txt-gray", ""))
    def calc_pct(valor):
    ...
    for idx, (num_sem, val_sem) in enumerate(semanas_totales.items()):
        ...
        semanas_html += f'<div class="wk-box"><div class="wk-title">{titulos_semanas[idx] if idx < len(titulos_semanas) else f"Week {num_sem}"}</div><div class="wk-val {c_sem}">{s_sem}${val_sem:,.2f}<br><span style="font-size:{WEEKS_PCT_SIZE}px;">{s_sem}{calc_pct(val_sem):.2f}%</span></div></div>'

    st.markdown(f'<div class="weeks-container">{semanas_html}<div class="mo-box"><div class="mo-title">{TXT_MO}</div><div class="mo-val {cM}">{sM}${m_total:,.2f}<br><span style="font-size:{WEEKS_PCT_SIZE}px;">{sM}{calc_pct(m_total):.2f}%</span></div></div></div>', unsafe_allow_html=True)
    
# ==========================================
# 9. HISTORIAL EXPANDER
# ==========================================
st.markdown("<br>", unsafe_allow_html=True)
st.markdown('<div class="thin-line"></div>', unsafe_allow_html=True)
    ...
    if not trades_actuales:
        ...
        for clave, data in trades_ordenados:
            ...
            with st.expander(f"📅 {data['fecha_str']} | P&L: {simbolo}${pnl_val:,.2f}"):
                ...
                with c_ed1: nueva_fecha = st.date_input("Day", value=datetime(anio_t, mes_t, dia_t), key=f"f_{clave}")
                with c_ed2: nuevo_bal = st.number_input("Nuevo Balance", value=float(data['balance_final']), format="%.2f", key=f"b_{clave}")
                with c_ed3:
                    ...
                    for i, img_b64 in enumerate(imagenes_restantes):
                        with cols_img[i]:
                            ...
                            if st.button("🗑️ Delete", key=f"delimg_{clave}_{i}", use_container_width=True):
                                ...
                                st.rerun()
                else: st.caption("No hay imágenes guardadas.")
                ...
                with c_btn1:
                    if st.button("💾 SAVE TODAY'S CHANGES", key=f"save_{clave}", use_container_width=True):
                        if nuevas_imgs:
                            for img in nuevas_imgs: imagenes_restantes.append(f"data:{img.type};base64,{convertir_img_base64(img)}")
                        nueva_clave = (nueva_fecha.year, nueva_fecha.month, nueva_fecha.day)
                        if nueva_clave != clave: del db_usuario[ctx]["trades"][clave]
                        ...
                        db_usuario[ctx]["trades"][nueva_clave] = {
                            ...
                            "correcciones": data.get("correcciones", ""), "risk": data.get("risk", "0.5%"), "rrr": data.get("rrr", "B"),
                            "trade_type": data.get("trade_type", ""), "emociones": data.get("emociones", "")
                        }
                        st.rerun()
                with c_btn2:
