...
if True:
    ...
    .lbl-header {
        ...
        width: 100% !important;
        padding-left: 5px !important; /* Hace que inicie exactamente alineado al borde del botón */
    }
        ...
        if not modo_lectura:
            ...
            with st.expander("Agregar Nuevo Trade", expanded=False):
                ...
                with st.form(key="form_main_entry", clear_on_submit=True, border=False):
                    c_date, c_cant, c_det, c_link, c_upd, c_btn = st.columns([0.6, 0.6, 0.6, 1.0, 0.4, 0.8])
                    
                    with c_date:
                        st.markdown('<div class="lbl-header">Día y Hora:</div>', unsafe_allow_html=True)
...
with tab_galeria:
    ...
    cnt_funded = len([x for x in todas_imagenes if x[3] == "Funded"])
